...
export interface IMonumentsClient {
    ...
    getRecentDetails(monumentId: number): Observable<MonumentDto | null>;
...
export class MonumentsClient implements IMonumentsClient {
    ...
    getRecentDetails(monumentId: number): Observable<MonumentDto | null> {
        let url_ = this.baseUrl + "/api/Monuments/monuments/recent/details?";
        if (monumentId === undefined || monumentId === null)
            throw new Error("The parameter 'monumentId' must be defined and cannot be null.");
        else
            url_ += "MonumentId=" + encodeURIComponent("" + monumentId) + "&"; 
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetRecentDetails(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetRecentDetails(<any>response_);
                } catch (e) {
                    return <Observable<MonumentDto | null>><any>_observableThrow(e);
                }
            } else
    ...
    protected processGetRecentDetails(response: HttpResponseBase): Observable<MonumentDto | null> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
